...
import { alwaysLinks, loggedInLinks, loggedOutLinks, onlyBusiness, onlyAdmin } from '../../../../routes/myLinks';
...
export default function LinksVertical() {
    ...
    return (
        <Box>
            ...
            {((user?.isBusiness || user?.isAdmin) && loggedIn) && onlyBusiness.map((link: Link) => (
                <NavLink to={link.to} key={nextKey()}>{link.children}</NavLink>))}
